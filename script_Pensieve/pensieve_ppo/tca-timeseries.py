...
NN_MODEL = '/mydata/Pensieve-PPO-TCA/src/pretrain/nn_model_ep_80100_bus_my.pth' # None    
...
class TCA:
    ...
    #! The input of this function is the source and target data, and the form should be a n-dim matrix (np array), it will return data in the same form
...
def testing(epoch, nn_model, log_file):
    ...
    # run test script
    os.system('python /mydata/Pensieve-PPO-TCA/src/tca/tca_test.py --nn_model ' + nn_model
...
def choose_and_combine(src_traces_path, trg_traces_path):
    ...
    dataset1_cooked_time, dataset1_cooked_bw, target_names = load_trace.load_trace(trg_traces_path)
    ...
    for i in range(len_combined - 1): # 因为我把时刻做差了
        ...
        tca_tool = TCA(kernel_type='primal', dim=2, lamb=1, gamma=1) # tca_rel.TCA
        ...
        min_val = np.min(dataset1_cooked_time[i][1:])
        max_val = np.max(dataset1_cooked_time[i][1:])
        new_dataset[:, 0] = min_val + (new_dataset[:, 0] - new_dataset[:, 0].min()) * (max_val - min_val) / (new_dataset[:, 0].max() - new_dataset[:, 0].min())

        # 将数组保存成指定格式的文件
        combined_save_path = './combined/distri_0/'
        np.savetxt(combined_save_path + "combined_" + target_names[i], new_dataset, fmt="%.11f", delimiter="\t")
...
if __name__ == '__main__':
    ...
    # !!!  如果不重新生成数据，要注释这里
    combined_cooked_time, combined_cooked_bw = choose_and_combine('./amazon/','./facebook/')
    # save combined, 然后combined 保存到 `/mydata/Pensieve-PPO-TCA/src/tca/train/combined/distri_0_1/`
    load_trace.COOKED_TRACE_FOLDER = './combined/distri_0/'
